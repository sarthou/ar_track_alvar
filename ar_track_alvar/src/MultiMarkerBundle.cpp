...
void Est(cv::Mat& state, cv::Mat& estimation, void *param)
{
	...
	for(int i = 0; i < n_images; ++i)
	{
		...
		double tra[3];
		double rodr[3];
		cv::Mat mat_translation_vector(3, 1, CV_64F, tra);
		cv::Mat mat_rotation_vector(3, 1, CV_64F, rodr);
		...
		for(int j = 0; j < n_points; ++j)
		{
			...
			cv::Mat mat_object_points(1, 1, CV_64FC3, object_points);

			double proj[2]={0};
			cv::Mat mat_proj(1, 1, CV_64FC2, proj);
...
bool MultiMarkerBundle::Optimize(Camera *_cam, double stop, int max_iter, Optimization::OptimizeMethod method)
{
	...
	cv::Mat parameters_mask_mat(n_params, 1, CV_8U);
	cv::Mat measurements_mat = cv::Mat::zeros(n_meas, 1, CV_64F);
	cv::Mat weight_mat(n_meas, 1, CV_64F);
	...
	for(size_t i = 0; i < marker_indices.size(); ++i) {
		...
		for (int j=0; j<4; j++) {
			...
			if (marker_status[i] > 0) {
				parameters_mat.at<double>(index + 0, 0) = pointcloud[pointcloud_index(id, j)].x;
				parameters_mat.at<double>(index + 1, 0) = pointcloud[pointcloud_index(id, j)].y;
				parameters_mat.at<double>(index + 2, 0) = pointcloud[pointcloud_index(id, j)].z;
			} else {
	...
	for (size_t f=0; f < frames; f++) {
		...
		// Camera pose
		cv::Mat tra(3, 1, CV_64F, &(parameters_mat.at<double>(f*7+0)));
		cv::Mat qua(4, 1, CV_64F, &(parameters_mat.at<double>(f*7+3)));
		...
		for(size_t i = 0; i < marker_indices.size(); ++i) {
			...
			if (measurements.find(measurements_index(f,id,0)) != measurements.end()) {
				for (int j=0; j<4; j++) {
					...
					measurements_mat.at<double>(index + 0, 0) = measurements[measurements_index(f, id, j)].x;
          			measurements_mat.at<double>(index + 1, 0) = measurements[measurements_index(f, id, j)].y;
					n_measurements += 2;
				}
			} else {
				for (int j=0; j<4; j++) {
					//hop int index = f*(n_markers*4*2) + id*(4*2) + j*2;
					int index = f*(n_markers*4*2) + i*(4*2) + j*2;
					weight_mat.at<double>(index+0, 0) = 0.;
					weight_mat.at<double>(index+1, 0) = 0.;
				}
			}
		}
	}
	...
	optimization_error = optimization.Optimize(parameters_mat, measurements_mat, stop, max_iter, Est, 0, method,
	...
	for(size_t i = 0; i < marker_indices.size(); ++i) {
		...
		for (int j=0; j<4; j++) {
			//hop int index = frames*7 + id*(3*4) + j*3;
			int index = frames*7 + i*(3*4) + j*3;
			pointcloud[pointcloud_index(id, j)].x = parameters_mat.at<double>(index + 0, 0);
			pointcloud[pointcloud_index(id, j)].y = parameters_mat.at<double>(index + 1, 0);
			pointcloud[pointcloud_index(id, j)].z = parameters_mat.at<double>(index + 2, 0);
		}
	}
