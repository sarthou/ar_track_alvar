  ...
  void AddMarker(const char* id)
  {
    ...
    if (marker_type == 0)
    {
      ...
      else
      {
        ...
        if (maxy > new_maxy)
          new_maxy = maxy;
        cv::Mat new_img = cv::Mat(cv::Size(int(new_maxx - new_minx + 0.5),
                                           int(new_maxy - new_miny + 0.5)),
                                  CV_8UC1, cv::Scalar(255));
        img.copyTo(
            new_img(cv::Rect(int(minx - new_minx + 0.5),
                             int(miny - new_miny + 0.5), img.cols, img.rows)));
        img.release();
        img = new_img;
        int roi_x = int((posx * units) - (marker_side_len * units / 2.0) -
                        new_minx + 0.5);
        int roi_y = int((posy * units) - (marker_side_len * units / 2.0) -
                        new_miny + 0.5);
        int roi_t = -roi_y;
        int roi_b = -img.rows + int(marker_side_len * units + 0.5) + roi_y;
        int roi_l = -roi_x;
        int roi_r = -img.cols + int(marker_side_len * units + 0.5) + roi_x;
    ...
    {
      // Create and save MarkerArtoolkit marker (Note, that this doesn't support
      // multi markers)
...
int main(int argc, char* argv[])
{
  try
  {
    ...
    if (st.prompt)
    {
      ...
      std::cout << "  This is an example of how to use the 'MarkerData' and "
                   "'MarkerArtoolkit'"
                << std::endl;
      std::cout << "  classes to generate marker images. This application can "
                   "be used to"
                << std::endl;
      std::cout << "  generate markers and multimarker setups that can be used "
                   "with"
                << std::endl;
      ...
      std::cout << std::endl;
      std::cout << "    65535             marker with number 65535"
                << std::endl;
      std::cout << "    -f 65535          force hamming(8,4) encoding"
                << std::endl;
      std::cout << "    -1 \"hello world\"  marker with string" << std::endl;
      std::cout << "    -2 catalog.xml    marker with file reference"
                << std::endl;
      std::cout << "    -3 www.vtt.fi     marker with URL" << std::endl;
      std::cout << "    -u 96             use units corresponding to 1.0 unit "
                   "per 96 pixels"
                << std::endl;
      std::cout << "    -uin              use inches as units (assuming 96 dpi)"
                << std::endl;
      std::cout << "    -ucm              use cm's as units (assuming 96 dpi) "
                   "<default>"
                << std::endl;
      std::cout << "    -s 5.0            use marker size 5.0x5.0 units "
                   "(default 9.0x9.0)"
                << std::endl;
      std::cout << "    -r 5              marker content resolution -- 0 uses "
                   "default"
                << std::endl;
      std::cout << "    -m 2.0            marker margin resolution -- 0 uses "
                   "default"
                << std::endl;
      std::cout << "    -a                use ArToolkit style matrix markers"
                << std::endl;
      std::cout << "    -p                prompt marker placements "
      ...
      if (st.prompt)
      {
        ...
        std::cout << "  units: " << st.units / 96.0 * 2.54 << " cm "
        ...
        while (true)
        {
          ...
          if (s.length() > 0)
            marker_id = atoi(s.c_str());
          if (marker_id < 0)
            break;
          std::cout << "  x position (in current units) [" << posx << "]: ";
          std::flush(std::cout);
          std::getline(std::cin, s);
          if (s.length() > 0)
            posx = atof(s.c_str());
          std::cout << "  y position (in current units) [" << posy << "]: ";
          std::flush(std::cout);
          std::getline(std::cin, s);
          if (s.length() > 0)
            posy = atof(s.c_str());
    ...
    else if (st.array)
    {
      ...
      if (s.length() > 0)
        cols = atoi(s.c_str());
      std::cout << "Rows distance : ";
      std::flush(std::cout);
      std::getline(std::cin, s);
      if (s.length() > 0)
        rows_distance = atof(s.c_str());
      std::cout << "Cols distance : ";
      std::flush(std::cout);
      std::getline(std::cin, s);
      if (s.length() > 0)
        cols_distance = atof(s.c_str());
      std::cout << "Column major (1 or 0) : ";
      std::flush(std::cout);
      std::getline(std::cin, s);
      if (s.length() > 0)
        column_major = atoi(s.c_str());
      std::cout << "First marker ID : ";
      std::flush(std::cout);
      std::getline(std::cin, s);
      if (s.length() > 0)
        first_id = atoi(s.c_str());
      if (!column_major)
      {
        for (int row = 0; row < rows; row++)
          for (int col = 0; col < cols; col++)
          {
            st.posy = row * rows_distance;
            st.posx = col * cols_distance;
